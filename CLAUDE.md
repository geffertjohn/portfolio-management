...
- **Surfaces:** `TipRanksPanel` on the stock Monitor tab, above `SecurityResearchPanel` — the whole street, then the AI team's own view, with row-click into `TipRanksAnalystModal` for one analyst's rank, stars, scorecard and recent calls across every name.
