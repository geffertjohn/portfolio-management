...
- **`position_sizing`** (monthly) and **`full_monitoring`** (quarterly) → `PositionSizingCheck` — an **allocations file upload** (YCharts export: col A ticker, col B weight in percent points; `.xls/.xlsx/.csv`). `parseActualAllocations` + `compareSizing` (`lib/positionSizingCompare.ts`) compare each holding's actual weight to its effective band and surface **only breaches** (actual outside lower/upper). Display-only — nothing is persisted. Effective bands come from `computePositionBands` (`lib/positionBands.ts`), which **mirrors the inline driftLower/driftUpper logic in `PortfolioDetailPage`** (explicit per-position limits win; else `roundToHalf(target × (1 ± drift_percentage/100))` from the resolved model portfolio; cash uses the model's cash limits) — keep the two in sync. `positions` + the resolved `modelPortfolio` are threaded from the page down through `PortfolioReviewsPanel` → modal.

### Quarterly full-monitoring per-holding assessment (`holding_reviews`)

The quarterly **`full_monitoring`** task renders `HoldingMonitorGrid`: the five review *areas* (Thesis · Fundamentals · Expectations · Valuation · Portfolio role) as guidance, then a per-holding table where each non-cash holding gets five categorical assignments — **Thesis Status** (intact/at_risk/broken), **Business Trend** (improving/stable/deteriorating), **Valuation** (attractive/fair/expensive), **Conviction** (high/medium/low), **Action** (add/hold/trim/exit/watchlist). Types/labels/options live in `lib/holdingReviews.ts` (DB stores lowercase tokens, app maps to labels).

These persist to a **dedicated `holding_reviews` table** (one row per `(review_log_id, security_id)`, unique on that pair; FKs to `portfolio_review_log` and `portfolio` both `ON DELETE CASCADE`; CHECK constraints on each enum), **not** the review's checklist jsonb — so conviction/thesis/action are queryable across reviews over time (feeds the annual "rebuild conviction rankings" task later). The grid is manual-entry (no FMP data aids yet) and **recorded-only** (no side effects into At-Risk/Watchlist). `markPortfolioReviewed()` now **returns the new `portfolio_review_log.id`**; the modal then calls `saveHoldingReviews(logId, …)` (skips fully-empty holdings). `PortfolioReviewLog` shows a per-review summary (action breakdown + thesis at-risk/broken count) from `fetchHoldingReviewsByPortfolio`. Query key: `holdingReviews(name)` — invalidate after a review mutation.

### `markPortfolioReviewed()` advances ONE timer

`markPortfolioReviewed({ portfolioName, cadence, checklist, notes, reviewDate, nextReviewAt })` upserts only that cadence's schedule row (`onConflict: 'portfolio_name,cadence'`) and inserts one frozen `portfolio_review_log` row, **returning its id**. `next_review_at` defaults to the cadence interval from the completion date (+1 month / +3 months / +1 year). `isOverdue`/`isDueSoon` are re-exported from `reviewSchedules.ts` (shared date helpers). Query keys: `portfolioReviewSchedules` (all) + `portfolioReviewSchedulesFor(name)`; invalidate both plus `portfolioReviews(name)` and `holdingReviews(name)` after a mutation.
