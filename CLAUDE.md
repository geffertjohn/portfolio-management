...
## Sell-Side Research Import (Raymond James PDFs)

Broker research PDFs uploaded on a **security's Documents tab** are parsed and recorded in **`external_research`** — the *street's* view (rating, target price, analyst commentary), as opposed to `research_reports`, which is the **AI team's own** output. Never conflate the two tables.

- **Upload → detect → confirm.** `DocumentsFolderPanel` takes an optional **`researchSecurityId`**; when set, a chosen PDF is sniffed for a research cover page before upload. A Raymond James report opens **`ResearchImportModal`** (review/correct the parsed fields, then save); anything else — including an unreadable or encrypted PDF — falls through to a plain document upload, so a parse failure never costs the file. pdfjs is **lazy-loaded** (`await import(...)`) inside that handler, so the portfolio/settings Documents surfaces don't pay for it.
- **Two-module split.** **`lib/researchPdfParse.ts`** is pure (React-free *and* pdfjs-free): it takes positioned text items and returns `ParsedResearch`. **`lib/pdfExtract.ts`** is the only pdfjs consumer — it turns a `File` into `PdfPageItems`. The split keeps the parser exercisable from a plain Node harness; don't import pdfjs into the parser.
- **Cover-page geometry is the parse strategy, not reading order.** On the letter-size RJ template the narrative column sits at **x≈36** and the metadata sidebar at **x≈419**, so `parseRaymondJames` splits items at `width * 0.62` — otherwise the sidebar's market-data rows interleave into the RECOMMENDATION paragraphs. Within a line, a gap > 2.5pt starts a new **run** (this is what separates a sidebar label from its right-aligned value); a gap > 0.6pt is a word space. Rotated glyphs (the per-recipient watermark) are dropped in `pdfExtract`.
- **What is captured:** ticker/exchange/industry, title, report type, published timestamp (with the printed timezone offset), rating label + RJ numeric scale (1 Strong Buy → 4 Underperform), target price **and the prior target** ("↑ old: $455.00"), price at publication, suitability, the analyst list, the market-data block, and the **RECOMMENDATION / VALUATION** narrative. `ParsedResearch.missing` lists anything not found → the row is stored `parse_status = 'partial'` and badged in the UI. Only page 1 is read; later pages are model tables and disclosures.
- **Display:** the Documents row renders rating/target inline for imported files, and **`ExternalResearchPanel`** ("Street Research") sits on the stock **Monitor** tab above the AI `SecurityResearchPanel` (and standalone on the fund/ETF page). It restates target upside against the **live** price by reusing `QUERY_KEYS.quote(symbol)` + `useLiveQuote`, so the quote is fetched once per page.
