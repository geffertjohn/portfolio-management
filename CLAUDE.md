...
- **`conviction_rankings`** (annual) → `ConvictionRankingSection` — tier reference (Tier 1–4 → meaning + target band 5–7/3–5/1–3/0–1%), then a per-holding **tier** select with current weight, the tier's target band, an **amber out-of-band flag** when the weight doesn't fit the tier, and a prior-conviction hint (most recent quarterly `conviction` from history).
